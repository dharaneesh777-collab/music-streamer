...
const Player = () => {
    const { currentTrack, isPlaying, togglePlay, closePlayer, audioRef } = useContext(PlayerContext);
    ...
    return (
        <div className="fixed bottom-0 w-full h-24 bg-gray-950 border-t border-gray-800 flex items-center px-4 md:px-6 justify-between z-50">
            <style>
                {`
            ...
            {/* Central Controls */}
            <div className="flex flex-col items-center w-auto md:w-1/3">
                <div className="flex gap-4 items-center">
                    <button className="hidden md:block text-gray-400 hover:text-white transition">⏮</button>
                    <button onClick={togglePlay} className="h-10 w-10 md:h-12 md:w-12 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 transition shadow-lg">
                        {isPlaying ? '⏸' : '▶'}
                    </button>
                    <button className="hidden md:block text-gray-400 hover:text-white transition">⏭</button>
                </div>
                
                <div className="hidden md:flex w-full mt-2 items-center gap-3">
                    <span className="text-xs text-gray-400 w-8 text-right font-mono">{currentTime}</span>
                    <div className="flex-1 h-1.5 bg-gray-600 rounded-full cursor-pointer flex items-center hover:h-2 transition-all relative" onClick={handleSeek} ref={progressBarRef}>
            ...
            {/* Mobile: Seek Bar pinned to absolute top of player. Desktop: Action buttons */}
            ...
            <div className="w-auto md:w-1/3 flex justify-end items-center gap-4 md:gap-6 md:pr-4">
                ...
                <button onClick={handleDownload} className="hidden md:block text-gray-400 hover:text-white transition text-xl" title="Download Offline">⬇️</button>
